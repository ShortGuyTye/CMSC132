...
public class TubeVideosManager implements TubeVideosManagerInt {
	...
	public boolean addVideoToDB(String title, String url, int durationInMinutes, Genre videoGenre) {
		try {
			Video newVideo = new Video(title, url, durationInMinutes, videoGenre);
	...
	public ArrayList<Video> getAllVideosInDB() {
		...
		for (int i = 0; i < videoDatabase.size(); i++) {
			temp.add(new Video(videoDatabase.get(i)));
		}
		return temp;
	}
	...
	public Video findVideo(String title) {
		...
		for (int i = 0; i < videoDatabase.size(); i++) {
			if (videoDatabase.get(i).getTitle().equals(title)) {
				Video temp = new Video(videoDatabase.get(i));
				return temp;
			}
		}
		return null;
	...
	public String[] getPlaylistsNames() {
		String[] allNames = new String[playlists.size()];
		for (int i = 0; i < playlists.size(); i++) {
			allNames[i] = playlists.get(i).getName();
		}
		return allNames;
	}
	...
	public Playlist getPlaylist(String playlistName) {
		...
		for (int i = 0; i < playlists.size(); i++) {
			if (playlists.get(i).getName().equals(playlistName)) {
				Playlist temp = new Playlist(playlists.get(i));
				return temp;
			}
		}
		return null;
	...
	public Playlist searchForVideos(String playlistName, String title, int maximumDurationInMinutes, Genre genre) {
		Playlist temp = new Playlist("Playlist");
		for (int i = 0; i < playlists.size(); i++) {
			if (playlistName == null || playlists.get(i).getName().equals(playlistName)) {
				for (int j = 0; j < playlists.get(i).getPlaylistVideosTitles().size(); j++) {
					if (title == null || playlists.get(i).getPlaylistVideosTitles().get(j).equals(title)) {
						if (maximumDurationInMinutes < 0 || findVideo(playlists.get(i).getName())
								.getDurationInMinutes() < maximumDurationInMinutes) {
							if (genre == null || findVideo(playlists.get(i).getName()).getGenre() == genre) {
								temp.addToPlaylist(playlists.get(i).getPlaylistVideosTitles().get(j));
	...
	public void genHTMLForPlaylist(String filename, String playlistName, boolean printFeedback) {
		...
		generateHTMLPagesWithBody(filename, htmlBody, printFeedback);
	}
	 ...
	 */
	private static void generateHTMLPagesWithBody(String filename, String body, boolean printFeedback) {
